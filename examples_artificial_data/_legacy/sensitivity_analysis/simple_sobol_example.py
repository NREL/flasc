...
if __name__ == "__main__":
    ...
    problem = {
        ...
        "bounds": [[0.058, 5.8], [0.05, 5.0], [0.08, 8.0]],
    }
    N = int(2000)

    fsba = fsasa.floris_sobol_analysis(fi=fi, problem=problem, calc_second_order=False)
    fsba.generate_samples(N)

    # Check if precalculated solutions exist
    root_path = os.path.dirname(os.path.abspath(__file__))
    df_path = os.path.join(
        root_path, "prec_solutions_calc2ndorder%s_N%04d.ftr" % (str(fsba.calc_second_order), N)
    )
    if os.path.exists(df_path):
        ...
        df = pd.DataFrame({"samples_x": fsba.samples_x, "samples_y": samples_y})
