...
for ti in range(len(fi.layout_x)):
    ...
    df = dfm.set_ws_by_upstream_turbines_in_radius(
        df=df, df_upstream=df_upstream, turb_no=ti, x_turbs=fi.layout_x,
        y_turbs=fi.layout_y, max_radius=5000., include_itself=True)
    df = dfm.set_ti_by_upstream_turbines_in_radius(
        df=df, df_upstream=df_upstream, turb_no=ti, x_turbs=fi.layout_x,
        y_turbs=fi.layout_y, max_radius=5000., include_itself=True)
    ...
    # Add 'pow_ref' columns defining reference power production
    df = dfm.set_pow_ref_by_upstream_turbines_in_radius(
        df=df, df_upstream=df_upstream, turb_no=ti, x_turbs=fi.layout_x,
        y_turbs=fi.layout_y, max_radius=5000., include_itself=False)

    df_fi = dfm.set_pow_ref_by_upstream_turbines_in_radius(
        df=df_fi, df_upstream=df_upstream, turb_no=ti, x_turbs=fi.layout_x,
        y_turbs=fi.layout_y, max_radius=5000., include_itself=False)

    # Test the turbines in proximity of reference turbine (ti)
    test_turbines = ftools.get_turbs_in_radius(
        x_turbs=fi.layout_x, y_turbs=fi.layout_y,
        turb_no=ti, max_radius=5000., include_itself=False)
    ...
    while current_time <= end_time:
        ...
        df_bias = df_bias.append({'ref_turbine': ti,
        ...
        print('ti = %d. Current time = %s. Estimated wd bias = %.1f degrees'
              % (ti, str(current_time), wd_bias))

        # Produce and save energy ratio figures
        fig_path_and_prefix = os.path.join(
            out_path,
            'ti%d_%s_energyratio' % (ti, str(current_time))
            )
        fsc.plot_energy_ratios(save_path=fig_path_and_prefix)
        plt.close('all')
