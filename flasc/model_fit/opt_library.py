...
def sweep_across_sectors(
    ...
    Args:
        ...
        n_points (int, optional): Number of points to evaluate in the parameter space.
            Defaults to 10.
    ...
    for wd_idx, wd in enumerate(wd_sectors):
        ...
        res = sweep_sector(mf, wd, wind_radius=wind_radius, n_points=n_points, verbose=verbose)
...
def sweep_opt_pair(
    ...
    # Pass this through to sweep_opt_sequential
    return sweep_opt_sequential(
        mf_, n_points=n_points, turbine_groupings=turbine_groupings, verbose=verbose
    )
...
def sweep_opt_sequential(mf: ModelFit, n_points=10, turbine_groupings=None, verbose=False) -> dict:
